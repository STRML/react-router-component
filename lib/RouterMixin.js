...
var RouterMixin = {
  ...
  navigate: function(path, navigation, cb) {
    ...
    var strippedPath = this.stripPath(path);
    if (path !== strippedPath) {
        this.context.routers[1].navigate(strippedPath, navigation, cb);
        return
    }
  ...
  /**
   * Returns path for parent router.
   *
   * This function is called by navigate.
   *
   * @private
   *
   * @param {String} path
